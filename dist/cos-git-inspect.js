...
import { DIFF_STDOUT_LIMIT, STATUS_STDOUT_LIMIT, runGit, } from './git-process.js';
...
async function canonicalDirectory(value) {
    ...
    if (!info.isDirectory())
        throw new Error(`Repository path is not a directory: ${canonical}`);
    return canonical;
}
    ...
    static async create(repoArgs) {
        ...
        for (const value of repoArgs) {
            ...
            if (comparable(canonicalTopLevel) !== comparable(canonical)) {
                throw new Error(`Allowed repository must name its top-level directory: ${canonical} (top level is ${canonicalTopLevel}).`);
            }
            roots.set(comparable(canonical), canonical);
        }
        return new AllowedRepositories(roots);
    }
    async requireAllowed(repoPath) {
        ...
        if (!allowed)
            throw new Error(`Repository is not allowed: ${canonical}`);
        return allowed;
    }
    list() {
...
export function parsePorcelainStatus(output, repoPath) {
    ...
    for (let index = 0; index < records.length; index += 1) {
        ...
        if (record.startsWith('# branch.ab ')) {
            ...
            if (!match)
                throw new Error('Git returned an invalid branch count record.');
        ...
        if (code === '??') {
            ...
            if (x !== '.' && x !== ' ' && x !== '!' && x !== '?')
                staged.push(change(x));
            if (y !== '.' && y !== ' ' && y !== '!' && y !== '?')
                unstaged.push(change(y));
        }
    }
    return { repoPath, branch, staged, unstaged, untracked, conflicts };
}
export async function getGitStatus(allowed, repoPath, runOptions = {}) {
    const repo = await allowed.requireAllowed(repoPath);
    const result = await runGit(repo, ['status', '--porcelain=v2', '-z', '--branch', '--untracked-files=all'], { stdoutLimit: STATUS_STDOUT_LIMIT, ...runOptions });
    return parsePorcelainStatus(result.stdout, repo);
}
function normalizePaths(repoPath, requestedPaths) {
    if (!requestedPaths)
        return [];
    if (requestedPaths.length > 256)
        throw new Error('Diff paths cannot contain more than 256 entries.');
    const normalized = [];
    let argumentBytes = 0;
    for (const requested of requestedPaths) {
        if (!requested || requested.includes('\0'))
            throw new Error('Diff paths must be non-empty.');
        if (path.isAbsolute(requested))
            throw new Error(`Diff paths must be repository-relative: ${requested}`);
        ...
        if (argumentBytes > 64 * 1024)
            throw new Error('Diff paths exceed the command-line size limit.');
        normalized.push(value);
    }
    return normalized;
}
export async function getGitDiff(allowed, repoPath, staged = false, requestedPaths, runOptions = {}) {
    const repo = await allowed.requireAllowed(repoPath);
...
//# sourceMappingURL=cos-git-inspect.js.map